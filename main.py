...
bot = TeleBot(token="7157644857:AAFF5YNsgIzFuGYvcf5elKvsRS3f6Y3dJkM",num_threads=45,threaded=True,skip_pending=True,parse_mode='html', disable_web_page_preview=True) # توكن بوت الرشق الاساسي هنا
if not db.get('accounts'):
    db.set('accounts', [])
    pass

admin = 5108562302 #الادمن
if not db.get("admins"):
    db.set('admins', [admin, 5108562302])
